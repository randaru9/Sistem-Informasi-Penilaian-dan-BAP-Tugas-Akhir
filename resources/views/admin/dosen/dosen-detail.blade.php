    ...
    <div class="bg-white ring-2 ring-blue1 rounded-[10px] w-full h-full overflow-y-auto">
        ...
        <div class="w-full px-5 flex py-2 gap-2">
            <div class="w-1/2">
                ...
                <a href="{{ route('dosen-ubah-katasandi', ['id' => $data['id']]) }}"
        ...
        <div class="w-full px-5 flex py-2 gap-2">
            <form action="{{ route('dosen-update-koordinator', ['id' => $data['id']]) }}" method="POST" class="w-1/2">
                @csrf
                <label for="hapus_akun" class="block mb-2 text-base text-[#000000] font-poppins font-normal">Koordinator
                    TA</label>
                @if ($data['is_koordinator'] == 1)
                    <p class="block mb-2 text-sm text-gray-500 font-poppins font-normal"> Pengguna Ini Adalah Koordinator TA</p>
                @else
                <button type="submit"
                    class="bg-gold text-white hover:bg-white hover:ring-2 hover:ring-gold hover:text-gold px-4 py-2 w-fit rounded-[5px] font-poppins text-base">
                ...
                @if (Session::has('koordinator'))
                    <p class="text-red-500">{{ Session::get('koordinator') }}</p>
                @endif
            </form>
        </div>
        <div class="w-full px-5 flex py-2 gap-2">
    ...
    <div id="konfirmasi_hapus_akun" tabindex="-1"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="{{route('hapus-pengguna-dosen', ['id' => request()->query('id')])}}" method="POST" class="p-4 md:p-5 text-center">
                    @csrf
