@php
    $breads = [
        ...
        [
            'href' => '/dosen/profil/verifikasi-email',
            'title' => 'Verifikasi Email',
        ],
    ];
@endphp

<x-layout-dosen :$breads title="Verifikasi Email">
    <div class="bg-white ring-2 ring-blue1 rounded-[10px] w-full h-full overflow-y-auto">
        <form action="{{route('profil-verifikasi-email-dosen-post')}}" method="POST">
            @csrf
            ...
            <div class="w-full px-5 flex py-2 gap-2">
                <div class="w-1/2 flex space-x-2">
                    ...
                    <a class="text-sm text-red-500 font-poppins font-normal" href="{{route('resend-otp-dosen')}}">Kirim ulang</a>
                </div>
            </div>
