...
class SeminarController extends Controller
{
    ...
    public function RekapNilaiExport(ExportRekapNilai $request)
    {
        ...
        $data = Seminar::query()->where('jenis_seminar_id', $request->safe()->jenis)->whereHas('Penilaians', function ($query) {
            $query->where('status_penilaian_id', '!=', 2);
        }, '=', 4)->whereBetween('tanggal', [$request->safe()->tglAwal, $request->safe()->tglAkhir])->with(['Penggunas', 
        'Penilaians'])->get();
